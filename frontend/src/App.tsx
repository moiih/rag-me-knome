...
import { faFileText, faComments } from '@fortawesome/free-solid-svg-icons';
...
function App() {
	...
	const handleSendMessage = async (userText: string): Promise<void> => {
		...
		try {
		const response = await fetch('https://literate-xylophone-qv7w46v965xr39jgg-8000.app.github.dev/api/chat', {
			method: 'POST',
			...
			<div className='min-h-dvh w-full flex flex-row overflow-hidden bg-slate-500 bg-slate-500'>
				...
				<div className='flex-1 min-h-screen flex flex-col gap-1 justify-between items-center rounded-xl border-2 border-red-300'>
					...
					<div className='flex-shrink-0 pt-3'>
						...
						{/* ------------ Toogle Switch for Chat UI ------------ */}
					<div className="absolute top-4 right-4 sm:top-6 sm:right-6 md:top-8 md:right-8 z-50">
						<button
							onClick={handleActiveChat}
							type="button"
							aria-label="Toggle between Content and Chat"
							className="relative flex items-center justify-between w-20 h-10 p-1 bg-gray-200 rounded-full cursor-pointer select-none transition-colors duration-300 shadow-inner"
						>
							{/* 1. Moving Pill Indicator Behind Icons */}
							<div
								className={`absolute top-1 bottom-1 w-8 rounded-full shadow transition-all duration-300 ease-out ${
								activeChat 
									? 'left-[calc(100%-2.25rem)] bg-green-500' // Right side / Chat (Green)
									: 'left-1 bg-blue-500'                     // Left side / Content (Blue)
								}`}
							/>

							{/* 2. Left Icon (Content) */}
							<div className={`z-10 flex items-center justify-center w-8 h-8 transition-colors duration-300 ${
								!activeChat ? 'text-white' : 'text-gray-500'
							}`}>
								<FontAwesomeIcon icon={faFileText} className="text-sm sm:text-base" />
							</div>
